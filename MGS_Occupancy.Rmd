...
umf <- readRDS('./Data/UMFs/umf_05152025.Rds')
...
umf_ind_adults <- readRDS("./Data/UMFs/umf_independentCams_adults.Rds")
...
m1_adults_indCams_noEnv <- occu(~ scOrdinal + scOrdinal2 ~ 1,
                 data = umf_ind_adults,
                 linkPsi = "logit")
...
```{r all-animals-all-cams-all-covs}
...
## Model 2: 
  - adults only
  - independent cameras only
  - environmental covs: Dist to Stream; Habitat Class (raster)
  - Detection effect: date terms
  - Occupancy effects: Env Covs


```{r adults-independentCams-allCovs}

adults_only_ind_cams_envCovs <- occu(~scOrdinal + scOrdinal2
                                     ~ Habitat_Class + Dist_to_Stream,
                                     data = umf_adultsOnly_indCams_envCovs,
                                     linkPsi = "logit")

summary(adults_only_ind_cams_envCovs)

```

This model has large or missing SE values, which is likely due to the sparsity of data (i.e., too few observations).

I think the next step is to use only adults; all cameras. 

## Model 3: 
  - adults only
  - all cameras
  - environmental covs: Dist to Stream and Habitat Class (raster)
  - Detection effect: date terms
  - Occupancy effect: Env Covs (fixed), camera name (random)

```{r adults-only-all-cams-allCovs, message=FALSE, warning=FALSE}

m3_adults_allCams_envCovs <- occu(~scOrdinal + scOrdinal2
                                     ~ Habitat_Class + Dist_to_Stream + (1|camera_name),
                                     data = umf_adultsOnly_allCams_envCovs,
                                     linkPsi = "logit")

summary(m3_adults_allCams_envCovs)

```

Even though none of the environmental covs are significant, let's see what the occupancy looks like:

```{r}

all_adults_pred_occu <- predict(m3_adults_allCams_envCovs, type = "state")
all_adults_pred_det <- predict(m3_adults_allCams_envCovs, type = "det")


cat("The mean 95% CI is:", mean(all_adults_pred_occu$lower), "-", mean(all_adults_pred_occu$upper))

cat("Range (occupancy probability):", range(all_adults_pred_occu$Predicted))
cat("Mean (occupancy probability):", mean(all_adults_pred_occu$Predicted))

cat("Range (detection probability):", range(all_adults_pred_det$Predicted))
cat("Mean (detection probability):", mean(all_adults_pred_det$Predicted))

```

Here we are again with an enormous CI, which means our model has too much uncertainty. This may be a function of too many variables for the amount of data.

A reasonable next step might be to eliminate the habitat classes, because each is estimated separately, and use only distance to dry stream bed.

## Model 4
  - Adults only
  - all cameras
  - Env Covs: Dist to Streams only
  - Detection Covs: date terms
  - Occupancy Covs: distance to streams (fixed), camera name (random)

```{r model4-allCams-adults-dist2streams}

m4_adults_allCams_streamsOnly <- occu(~scOrdinal + scOrdinal2
                                     ~ Dist_to_Stream + (1|camera_name),
                                     data = umf_adultsOnly_allCams_streams,
                                     linkPsi = "logit")

summary(m4_adults_allCams_streamsOnly)
```
Woohoo! Distance to Streams is significant! 

```{r m4-summaries}

all_adults_pred_occu <- predict(m4_adults_allCams_streamsOnly, type = "state")
all_adults_pred_det <- predict(m4_adults_allCams_streamsOnly, type = "det")
...
Ergh, the upper 95% CI goes to 1 for all cameras, and the max lower interval ~ 0. Not good. But still promising that distance to streams was significant.

## Model 5
  - Independent cameras only
  - Adults only
  - Detection Covs: date terms
  - Occupancy Covs: distance to streams
  
  
```{r}

m5_adults_indCams_streamsOnly <- occu(~scOrdinal + scOrdinal2
                                     ~ Dist_to_Stream,
                                     data = umf_adultsOnly_indCams_streamsOnly,
                                     linkPsi = "logit")

summary(m5_adults_indCams_streamsOnly)

```
  
```{r m5-summaries}

all_adults_pred_occu <- predict(m5_adults_indCams_streamsOnly, type = "state")
all_adults_pred_det <- predict(m5_adults_indCams_streamsOnly, type = "det")


cat("The mean 95% CI is:", mean(all_adults_pred_occu$lower), "-", mean(all_adults_pred_occu$upper)) # YASSSSS KWEEEEENNNNN
...
Let's try scaling the distance to streams

## Model 6: same as 5 but scaled distance to streams

```{r}


m6_adults_indCams_streamsOnly_scaled <- occu(~scOrdinal + scOrdinal2
                                     ~ Dist_to_Stream,
                                     data = scaled,
                                     linkPsi = "logit")

summary(m6_adults_indCams_streamsOnly_scaled)

```

```{r}

all_adults_pred_occu <- predict(m6_adults_indCams_streamsOnly_scaled, type = "state")
all_adults_pred_det <- predict(m6_adults_indCams_streamsOnly_scaled, type = "det")
...
# Save occupancy predictions so can superimpose ontp cameras

occupancyProbs <- all_adults_pred_occu

occupancyProbs <- occupancyProbs %>%
  mutate(camID = row_number())

```

# Model 7: Detection covariates only
