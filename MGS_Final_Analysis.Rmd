...
umf <- readRDS("./Data/UMFs/umf_CORRECT_06022025.Rds")

umf_recentered <- readRDS("~/Repos/MGS/Data/UMFs/recentered_dates_06062025.Rds") # this is a umf

finalDF <- readRDS("./Data/indCams_adults_envCovs_06022025.Rds")

weights_matrix <- readRDS("./Data/Detection/detection_weights_matrix.Rds")
...
## Model 2: Detection Trend Only (date terms: linear + quadratic); unweighted

```{r det-trend-only-linear-quadratic-unweighted, warning=FALSE}
...
## Model 3: Detection Trend Only - linear term only, unweighted

```{r det-trend-only-linear-unweighted}
...
## Model 4: Detection Trend Only, Linear term only, weighted

```{r det-trend-only-linear-weighted}
...
## Model 3: Detection Trend Only (date terms) + Occupancy Trend (distance to stream)
